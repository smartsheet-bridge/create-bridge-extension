...
And add a file called `.tsconfig` in the root folder populated like so.
...
The file path of the entry point relative to the `package.json` file must match the `main` property within. E.g. if you are creating a JavaScript extension and you've created a `package.json` file with the value of property `main` set to `src/index.js` then this file must exist at the path `src/index.js`.
