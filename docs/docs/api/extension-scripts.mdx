...
| `--staticDependencies` | Names of dependencies to be bundled with Extension code as-is unprocessed by the bundling tool.                                 |
| `--staticAssets`       | Glob patterns for static files to be bundled with Extension code. Use to include files not referenced via `require` statements. |
| `--entrypoint`         | Entrypoint for main extension function.                                                                                         |
...
| `--specFile`       | Filename of extension specification file.                         |
...
Some inline options have an alias that shortens the format to `-[alias] [value]`. E.g. `--minutes=5` becomes `-m 5`.
...
Options can be configured via environment variables (in your development environment) and come in the format of uppercase and prepended with `EXTENSION_`. E.g. `--url=[account url]` becomes `EXTENSION_URL:[url]`. Environment variables take priority over all but inline options.
