...
Returned data from `onInstall` doesn't automatically render in the UI of an external system. In order for the UI to render the extra data returned they must be declared in the [Spec](/bridge/spec) file.
