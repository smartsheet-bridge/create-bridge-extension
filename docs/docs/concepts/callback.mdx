...
A "callback" is the same as a "function" but it cannot be called by an external system. These functions are called by the extension platform itself at specific times and expect specific responses.

## Install/Uninstall

### `onInstall`

When an extension is [_installed_](./lifecycle#installation) by a user of an external system the platform will trigger the `onInstall` callback if one exists. This is particularly useful if you wish to gather inputs from the user that are required to integrate with another service.
...
Whatever is returned from the `onInstall` function is saved with the particular instance of the extension and can be accessed in any function via [`context.settings`](./function#context).
...
Returned data from `onInstall` doesn't automatically render in the UI of an external system. In order for the UI to render the extra data returned they must be declared in the [Spec](./spec) file.

:::

### `onUninstall`

Likewise, when an extension is [_uninstalled_](./lifecycle#uninstalling) by a user of an external system the platform will trigger the `onUninstall` callback if one exists. There are no inputs given and no data is expected to be returned.
