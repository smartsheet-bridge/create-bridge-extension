import createInstance, {
  isBridgeHTTPInstance,
} from './createBridgeHttpInstance';
import {
  BridgeHTTPInstance,
...
export const parseAccountURL = (accountURL: string): AccountURL => {
  const url = new URL(
    !accountURL.startsWith('http') ? `https://${accountURL}` : accountURL
  );
...
export function createAPICall<Options extends { [key: string]: any }, R = any>(
  fn: (
    instance: BridgeHTTPInstance
  ) => (
    options: Options,
    config: BridgeHTTPRequestConfig
  ) => Promise<BridgeHTTPResponse<R>>
): {
  (instance: BridgeHTTPInstance): (
    options: Options,
    config?: BridgeHTTPRequestConfig
